...
function appendControllers(directory) {
    ...
    let files = fs.readdirSync(path.join(__dirname, directory));
    let jsFiles = files.filter(f => f.endsWith('.js'));

    for (let jsFile of jsFiles) {
        console.log(`loading ${jsFile}...`);
        let controller = require(path.join(__dirname, directory, jsFile));
...
function publicDirectory(directory, route, router) {
    let result = {};
    let localDirectory = path.join(__dirname, directory);
